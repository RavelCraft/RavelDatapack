...
public class GrilledBrownMushroomItem {
    public static void register(CustomRegistry.CustomRegistryAdder adder, String namespaceKey) throws EasyDatapackException {
        CustomItem item = CustomFoodItem.builder(namespaceKey, ChatColor.WHITE + "Grilled Brown Mushroom", Material.COOKED_BEEF, TexturePath.item(namespaceKey), 3, 7.2f)
                .build();

        adder.register(item, new FurnaceRecipe(item.getNamespacedKey(), item.createItemStack(), Material.BROWN_MUSHROOM, 0.10f, 100));
    }
}
