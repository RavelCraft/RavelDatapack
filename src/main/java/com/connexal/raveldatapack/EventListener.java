...
public class EventListener implements Listener {
    ...
    public void onPrepareAnvilEvent(PrepareAnvilEvent event) {
        ...
        if (second == null || second.getAmount() > 1 || !RavelDatapack.getEnchantmentManager().isEnchantable(second)) return;
    ...
    private void updateGrindstone(Inventory inventory) {
        RavelDatapack.getInstance().getServer().getScheduler().runTask(RavelDatapack.getInstance(), () -> {
            ...
            curses.forEach((excellentEnchant, level) -> {
                RavelDatapack.getEnchantmentManager().enchantItemStack(result, excellentEnchant, level, true);
            });
            RavelDatapack.getEnchantmentManager().updateItemLoreEnchants(result);
        });
    }
