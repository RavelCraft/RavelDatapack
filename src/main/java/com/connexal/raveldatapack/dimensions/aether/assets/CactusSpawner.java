...
public class CactusSpawner {
        ...
        if (random.nextInt(2) == 0) {
            ...
            if (random.nextBoolean()) { //X or Z axis
                if (random.nextBoolean()) { //X +
                    spawnBranch(x + 1, tmpY, z, limitedRegion, random);
                }
                if (random.nextBoolean()) { //X -
                    spawnBranch(x - 1, tmpY, z, limitedRegion, random);
                }
                ...
                if (random.nextBoolean()) { //Z +
                    spawnBranch(x, tmpY, z + 1, limitedRegion, random);
                }
                if (random.nextBoolean()) { //Z -
                    spawnBranch(x, tmpY, z - 1, limitedRegion, random);
                }
    ...
    private static void spawnBranch(int x, int y, int z, LimitedRegion limitedRegion, Random random) {
