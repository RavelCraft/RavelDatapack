...
public class DimensionManager implements Listener {
    ...
    public void registerDimension(CustomDimension dimension) {
        ...
        if (RavelDatapackAPI.getConfig().contains("dimensions." + dimension.getName())) {
            if (!RavelDatapackAPI.getConfig().getBoolean("dimensions." + dimension.getName())) {
                return;
            }
        } else {
            RavelDatapackAPI.getConfig().set("dimensions." + dimension.getName(), false);
    ...
    public void createWorlds() {
        long seed = 0;
        World world = RavelDatapackAPI.getServer().getWorld("world");
        if (world != null) {
            seed = world.getSeed();
        }
